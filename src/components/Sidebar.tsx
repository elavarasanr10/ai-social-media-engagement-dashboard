...
const nav: { label: SectionId; icon: typeof LayoutDashboard; badge?: string }[] = [
      ...
      <nav className="flex-1 px-3 py-5 space-y-1">
        ...
        {nav.map((item) => {
          ...
          return (
            ...
            >
              <item.icon className={`w-[18px] h-[18px] ${isActive ? 'text-cyan-400' : 'text-slate-500 group-hover:text-slate-300'}`} />
              <span className="flex-1 text-left">{item.label}</span>
              {item.badge && (
                <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-md bg-cyan-500/20 text-cyan-300">{item.badge}</span>
              )}
              {isActive && <span className="w-1.5 h-1.5 rounded-full bg-cyan-400 shadow shadow-cyan-400/50" />}
            </button>
          );
        })}
      </nav>
